...
function parseItems(source, unit) {
  return [
    ...source.matchAll(
      /\{\r?\n\s+id: "([^"]+)",[\s\S]*?label: "([^"]+)",[\s\S]*?cefr: "([^"]+)",[\s\S]*?description: (PLACEHOLDER_DESCRIPTION|"([^"]*)"),[\s\S]*?topic: "([^"]+)"/g
    ),
  ].map((match) => ({
    unit,
    id: match[1],
...
for (const file of selectedFiles) {
  items.push(
    ...parseItems(await readFile(join(unitsDirectory, file), "utf8"), file.replace(/\.ts$/, ""))
  );
}
...
for (const item of items) {
  ...
  if (item.description && item.description[0] !== item.description[0].toUpperCase())
    flags.push("not-capitalized");
  if (item.description && /known as|needs manual|undefined/i.test(item.description))
    flags.push("generated-copy");
  ...
  if (!item.placeholder && (words < min || words > max))
    flags.push(`word-count-${words}-target-${min}-${max}`);
  if (item.description && normalize(item.description).includes(normalize(item.label)))
    flags.push("headword-leakage");
...
const result = {
  ...
  findings,
  reviewRule:
    "Use Cambridge and Oxford links to confirm the pictured sense, part of speech, CEFR fit, and regional usage. Write original WordPix copy; never paste dictionary text or examples.",
};

if (json) console.log(JSON.stringify(result, null, 2));
else {
  console.log(
    `Vocabulary review: ${result.words} words, ${result.placeholders} placeholders, ${findings.length} findings.`
  );
  for (const finding of findings) {
    console.log(
      `${finding.unit}/${finding.id}\t${finding.flags.join(",")}\tCambridge: ${finding.sources.cambridge}\tOxford: ${finding.sources.oxford}`
    );
  }
}

if (findings.some((finding) => finding.flags.some((flag) => !flag.startsWith("placeholder"))))
  process.exitCode = 1;
