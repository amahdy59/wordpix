  ...
  return (
    <div className="flex flex-col gap-6 p-5 md:p-8">
      ...
      {activeWorlds.map((world) => {
        const wordsPracticedCount = world.vocabulary.filter((w) => progress.wordMastery[w.id]).length;
        const totalWords = world.vocabulary.length;
        const progressPercent = Math.round((wordsPracticedCount / totalWords) * 100);

        return (
          <section aria-label={`Active World: ${world.name}`} key={world.id}>
              ...
              <div className="flex-1 flex flex-col justify-between gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <div className="flex justify-between items-center text-xs font-sans font-semibold">
                    ...
                    <span className="text-primary font-bold">{progressPercent}% Complete ({wordsPracticedCount}/{totalWords})</span>
                  </div>
                  <div
                ...
                >
                  ...
                  <span>Enter {world.name} Lesson</span>
