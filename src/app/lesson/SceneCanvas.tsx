...
export const SceneCanvas = memo(function SceneCanvas({
  ...
  return (
    <div className="relative flex-1 md:flex-[3] flex flex-col bg-background" style={{ minHeight: "55vmin" }}>
      ...
      {/* Desktop header bar */}
      <div className="hidden md:flex items-center justify-between px-5 py-4 bg-wp-card border-b border-border shrink-0 z-10">
      ...
      {/* Main Scene Container - object-contain aspect-ratio aware */}
      <div className="relative flex-1 overflow-hidden flex items-center justify-center bg-slate-900/90 p-4 md:p-6" style={{ minHeight: "340px" }}>
        
        {/* Baseline 2D Interactive Scene Background (Fits Container, Object Contain) */}
        <div className="relative max-w-full max-h-full flex items-center justify-center overflow-hidden rounded-2xl border border-white/10 shadow-2xl">
          <img
            ...
            className="max-h-[65vh] w-auto max-w-full object-contain pointer-events-none rounded-2xl"
            src={imgDefaultScene}
          />
        ...
        {/* Selected Word Featured Preview Card (Un-cropped aspect ratio fit) */}
        {activeWord.img && (
          <div className="absolute inset-x-4 md:inset-x-8 bottom-6 z-20 flex justify-center pointer-events-none">
            <div className="bg-wp-card/95 backdrop-blur-xl border border-border/80 rounded-2xl p-4 shadow-wp-md flex items-center gap-4 max-w-xl w-full pointer-events-auto motion-safe:transition-all">
              <div className="size-20 md:size-24 rounded-xl overflow-hidden shrink-0 border border-border bg-muted flex items-center justify-center">
                <img
                  src={activeWord.img}
