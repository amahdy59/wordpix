  ...
  if (!recallConfig || prompts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center p-8 text-center max-w-xl mx-auto gap-4">
        ...
        <h2 className="text-xl font-black text-foreground">No Recall Required</h2>
        <p className="text-sm text-muted-foreground">
          This unit has no preceding spaced repetition tasks. You are ready to begin the warm-up!
        </p>
        <button
          type="button"
          onClick={onNext}
    ...
    <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full py-2">
      ...
      <div className="flex items-center justify-between gap-4">
        <span className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-primary">
          ...
          Stage 0 · Spaced Repetition Recall
        </span>
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest hidden sm:inline">
            Unit {recallConfig.sourceUnitNumber} Language
          </span>
        </div>
      </div>
      ...
      >
        <div>
          <div className="flex items-center gap-2">
            ...
            >
              0. Quick Recall · Unit {recallConfig.sourceUnitNumber}: {recallConfig.sourceUnitTitle}
            </h1>
          </div>
          <p className="mt-1 text-sm text-muted-foreground font-medium">
            ~{recallConfig.estimatedMinutes || 3} minutes · Active recall helps schedule your next
            spaced review.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:items-end gap-1 shrink-0">
          <span className="text-xs font-black text-foreground">
            {totalCompleted} of {prompts.length} Prompts Completed
          </span>
          ...
          <div className="flex items-center justify-between gap-2 mb-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-muted-foreground">
                Prompt {activeIdx + 1} of {prompts.length}
              </span>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-4">
            ...
            {showHint && currentPrompt.hint && (
              <div className="flex items-start gap-2 rounded-xl bg-primary/10 p-3.5 text-xs sm:text-sm font-semibold text-primary border border-primary/20">
                ...
                <span>Hint: {currentPrompt.hint}</span>
              </div>
            )}
              ...
              <div className="flex flex-col gap-4 rounded-2xl bg-card border border-border p-5 mt-2 shadow-wp-xs">
                ...
                <div className="pt-3 border-t border-border/80">
                  <p className="text-xs font-black uppercase tracking-wider text-muted-foreground mb-2">
                    How easily did you recall this item?
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    ...
                    >
                      ...
                      <span className="text-[10px] font-semibold opacity-80">Review soon</span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span className="text-[10px] font-semibold opacity-80">Review later</span>
                    </button>
                  </div>
                </div>
              </div>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="spaced-mastery-title" className="text-xl font-black text-foreground">
                Spaced Repetition Mastery Completed!
              </h2>
              <p className="text-sm font-semibold text-muted-foreground">
                You successfully reviewed all {prompts.length} target items from Unit{" "}
                {recallConfig.sourceUnitNumber}.
              </p>
            </div>
          </div>

          <p className="text-xs sm:text-sm font-medium text-muted-foreground leading-relaxed">
            Finishing all prompts helps schedule the next review for each word. Mistakes are an
            active part of the spaced recall cycle and keep your vocabulary retention sharp.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-4 border-t border-border/80">
            ...
            >
              ...
              <span>Review Prompts Again</span>
            </button>
            ...
            >
              <span>Begin Lesson Warm-Up</span>
      ...
      {!isAllCompleted && prompts.length > 1 && (
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            >
              Remaining Prompts ({prompts.length - totalCompleted})
            </h3>
            <span className="text-xs font-bold text-muted-foreground">
              Unit {recallConfig.sourceUnitNumber}
            </span>
          </div>
              ...
              return (
                ...
                >
                  ...
                  {isDone ? (
                    ...
                    <span className="text-[11px] font-black text-primary uppercase">Active</span>
                  ) : (
                    <span className="text-[11px] font-medium text-muted-foreground">Queued</span>
                  )}
                </div>
              );
            })}
