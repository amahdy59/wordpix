...
describe("WordPix navigation and group lesson reducer", () => {
  ...
  it("starts a group lesson and advances/regresses step-by-step", () => {
    let state = reducer({ id: "lesson-entry" }, { type: "START_LESSON", lessonId: "furniture-1" });
    expect(state).toMatchObject({ id: "lesson", lessonId: "furniture-1", step: 0, attempts: [] });

    state = reducer(state, { type: "LESSON_NEXT" });
    expect(state).toMatchObject({ id: "lesson", step: 1, lessonId: "furniture-1" });

    state = reducer(state, { type: "LESSON_PREVIOUS" });
    expect(state).toMatchObject({ id: "lesson", step: 0, lessonId: "furniture-1" });
  ...
  it("records attempts and carries them into group completion results", () => {
    ...
    expect(state).toMatchObject({ id: "lesson-complete", lessonId: "furniture-1" });
