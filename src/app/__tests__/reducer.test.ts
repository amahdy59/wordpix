...
describe("WordPix navigation and group lesson reducer", () => {
  ...
  it("starts a group lesson and advances/regresses step-by-step", () => {
    let state = reducer({ id: "lesson-entry" }, { type: "START_LESSON", lessonId: "essential-furniture" });
    expect(state).toMatchObject({ id: "lesson", lessonId: "essential-furniture", step: 0, attempts: [] });

    state = reducer(state, { type: "LESSON_NEXT" });
    expect(state).toMatchObject({ id: "lesson", step: 1, lessonId: "essential-furniture" });

    state = reducer(state, { type: "LESSON_PREVIOUS" });
    expect(state).toMatchObject({ id: "lesson", step: 0, lessonId: "essential-furniture" });
  ...
  it("records attempts and carries them into group completion results", () => {
    ...
    expect(state).toMatchObject({ id: "lesson-complete", lessonId: "essential-furniture" });
